...
mod stm32f40x {
    ...
    impl VolatileCell<u32> {
        ...
        pub fn modify(&self, offset: u8, width: u8, value: u32) {
            ...
            let r = self.read() & !(w_mask << offset);
            self.write(r | ((value & w_mask) << offset));
...
/*
    There is nothing to uncomment but I add it to the code. The loop has to be like this:
    loop{
        gpioa.ODR.write(gpioa.ODR.read() | (1 << 5));
        wait(10_000);
        gpioa.ODR.write(gpioa.ODR.read() & !(1 << 5));
...
//
/*
    The changes that you need to introduce are:
    1) Implementing the modify function
        impl VolatileCell<u32> {
        #[inline(always)]
        pub fn modify(&self, offset: u8, width: u8, value: u32) {
    ...
    2) Introducing this code in the loop:
    loop{
    gpioa.ODR.modify(5, 1, 0b1);
    wait(10_000);
    gpioa.ODR.modify(5, 1, 0b0);
