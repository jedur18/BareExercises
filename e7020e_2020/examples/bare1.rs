...
//
/*
    After passing the three breakpoints nothing appear in ITM output. Buf if we continue after this message:
    Breakpoint 4, rust_begin_unwind (info=0x20007f98)
    at /home/edu/.cargo/registry/src/github.com-1ecc6299db9ec823/panic-itm-0.4.1/src/lib.rs:46
   46  interrupt::disable();
   We will see the next output in the ITM output
*/

/*
   ITM output shows: panicked at 'attempt to add with overflow', examples/bare1.rs:23:9
   It is caused because variable x has its maximun value -1 at the begining. In the first iteration of the
   loop it reachs its maximum value, so in second iteration the variable overflows.
*/
...
=> 0x0800041a <+12>:	bkpt	0x0000
   ...
   0x08000434 <+38>:	movw	r0, #6144	; 0x1800
   0x08000438 <+42>:	movt	r0, #2048	; 0x800
   0x0800043c <+46>:	movw	r2, #6116	; 0x17e4
   0x08000440 <+50>:	movt	r2, #2048	; 0x800
   0x08000444 <+54>:	movs	r1, #28
   0x08000446 <+56>:	bl	0x8000552 <core::panicking::panic>
...
//
/*
     There are three instructions between the two bkpt:
     0x0800041c <+14>:	mov.w	r0, #4294967295	; 0xffffffff
     0x08000420 <+18>:	add	r4, sp, #4
     0x08000422 <+20>:	str	r0, [sp, #4]
*/
...
//
/*    
      The store instruction: str r0, [sp, #4]
*/
...
//
/*
      There is just one instruction:
      0x08000416 <+12>:	str	r0, [sp, #0]
*/
//    Where is the local variable stored?
//
/*
      It is stored in the stack at adress sp-0
*/
//    Is there now any reference to the panic handler?
//    If not, why is that the case?
//
/*    
      There is not reference to the panic handler in relase mode because its optimized (payment for this 
      optimization is that the addition is not checked). Contrary happens in 
      debug build where some code is generated to check if the addition wraps, 
*/
...
//
/*
      There are no instructions between the two bkpt instructions
*/
//    Where is the local variable stored?
//    What happened, and why is Rust + LLVM allowed to do that?
//
//    Using that ouput i can not see it.
//
